<template>
  <div class="max-w-md mx-auto mt-10 bg-white dark:bg-gray-800 p-6 rounded shadow">
    <h2 class="text-2xl font-bold mb-6 text-center">原料出库</h2>
    ...
    <form @submit.prevent="onSubmit" class="space-y-4">
      ...
      <div>
        <input v-model.number="form.quantity" type="number" min="0.01" step="0.01" class="input" placeholder="出库数量" required />
      </div>
      ...
      <button type="submit" class="btn-primary w-full bg-orange-600">确认出库</button>
...
async function onSubmit() {
  ...
  error.value = ''
  try {
    await fetch('http://127.0.0.1:8000/admin/inventory/out', {
      method: 'POST',
      headers: {
